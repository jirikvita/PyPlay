  ...
  def FillHistos(self, families):
      ...
      for family in families:
          for mem in family.GetMembers():
              ...
              if key != gDead:
                  #print('Age: {}'.format(mem.GetAge()) )
                  # TODO
                  #self._histos['age'][2].append(mem.GetAge())
                  pass
      for ikey in gKeys:
          ...
          for hkey in self._histos:
              ...
              if str(ikey) == hkey:
                  print('{} {}'.format(hkey, counts[hkey]))
                  # TODO
                  #self._histos[hkey][2][hkey] = counts[hkey]
              iterKey = MakeIterHkey(ikey)
              if hkey == iterKey:
                  # fill histos as function of nIter!
                  print(ikey, hkey, iterKey)
                  self._histos[iterKey][2][hkey] = counts[RemoveIterHkey(hkey)]
      #if setMax:
